...
export function* alphaBeta(
  ...
  for (const child of node.children) {
    const childValue: number = yield* alphaBeta(child, depth - 1, alpha, beta, !isMax, currentPath);
    ...
    yield {
      ...
      nodeId: node.id,
      description: `Update batas: Nilai=${value}, \u03B1=${alpha}, \u03B2=${beta}`,
      currentValue: value,
      alpha,
      beta,
      visitedIds: [...currentPath],
    ...
    if (alpha >= beta) {
      yield {
        ...
        type: StepType.PRUNE,
        nodeId: node.id,
        description: `PANGKAS (PRUNE)! \u03B1 (${alpha}) >= \u03B2 (${beta})`,
        alpha,
        beta,
        visitedIds: [...currentPath],
        activePath: currentPath,
